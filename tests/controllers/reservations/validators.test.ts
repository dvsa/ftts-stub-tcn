import {
  ...
  isSlotReserved,
  getSlotQuantity,
} from '../../../src/controllers/reservations/validators';
...
describe('Reservations validators', () => {
  describe('validateReservationRequest', () => {
    ...
    describe('returns false', () => {
      test('when called with an empty object', () => {
        expect(validateReservationRequest({} as ReservationsRequest)).toBe(false);
      });
  ...
  describe('getSlotQuantity', () => {
    test('returns 2 given a datetime with a time of 1pm (max quantity of 2)', () => {
      const reservedStartDateTime = '2020-07-02T13:00:00+0000';

      expect(getSlotQuantity(reservedStartDateTime, 15)).toBe(2);
    });

    test('returns quantity in request given a datetime with a time different from 1pm (unlimited quantity)', () => {
      const reservedStartDateTime = '2020-07-02T09:00:00+0000';

      expect(getSlotQuantity(reservedStartDateTime, 15)).toBe(15);
    });
  });
});
