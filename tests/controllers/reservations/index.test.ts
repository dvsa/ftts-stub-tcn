...
import { BadRequestError, ReservationNotValidError, TooManyRequestsError } from '../../../src/errors';
...
describe('httpTrigger', () => {
  ...
  describe('when called with POST - to make a reservation', () => {
    ...
    test('throws an error when encountering one that is unknown', () => {
      mockMakeReservation.mockImplementation(() => {
        throw new Error('Unknown Error');
      });

      expect(() => httpTrigger(context, httpReq)).toThrow(Error('Unknown Error'));
    });
  });
